...
#JAVA_MEM_OPTS="-server -Xmx1.5g -Xms1.5g -Xmn1024m -XX:PermSize=128m"
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.thinkjoy.swms.DubboProviderApplication > $STDOUT_FILE 2>&1 &
...
#    COUNT=`ps -f | grep java | grep "$DEPLOY_DIR" | awk '{print $2}' | wc -l`
#    fi
    COUNT=`netstat -tnl|grep 2088|wc -l`
    if [ $COUNT -gt 0 ]; then
        break
    fi
done
